...
describe(fromPlanetsActions.Types.TogglePlanetsFavouriteStatus, ()=> {
  it('should move the target', () => {
    const action = new fromPlanetsActions.TogglePlanetsFavouriteStatus(mockPlanet);
    const result = reducer({...initialState, favouritePlanets: {ids: [mockPlanet.name],entities: { [mockPlanet.name]: mockPlanet}}}, action);
...
describe(fromPlanetsActions.Types.LoadPlanetsSuccess, () => {
  it('should add payload to planets entity', () => {
    const action = new fromPlanetsActions.LoadPlanetsSuccess({results: [mockPlanet], page:undefined, count:undefined});
    const result = reducer(initialState, action);

    expect(result).toEqual({...initialState, count: undefined, page: undefined, planets: {ids: [mockPlanet.name],entities: { [mockPlanet.name]: mockPlanet}}})
  })
});

describe(fromPlanetsActions.Types.LoadPlanetsFavourites, () => {
  it('should add payload to fav planets entity', () => {
    const action= new fromPlanetsActions.LoadPlanetsFavouritesSuccess([mockPlanet]);
    const result = reducer(initialState, action);

    expect(result).toEqual({...initialState, favouritePlanets: {ids: [mockPlanet.name],entities: { [mockPlanet.name]: mockPlanet}}})
  });
});
...
describe(fromPlanetsActions.LoadPlanetDetailsSuccess, () => {
  it('should insert payload into planetsDetails property', () => {
    const action = new fromPlanetsActions.LoadPlanetDetailsSuccess(mockPlanet);
    const result = reducer(initialState, action);

    expect(result).toEqual({...initialState, loading:false, planetDetails: mockPlanet});
  });
});
